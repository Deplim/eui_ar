...
def solution2(n, path, order):
    ...
    # 노드 i 에서 시작하여 check,res 를 계속 다시 탐색했을때 i 로 돌아오는지 확인 
    # (i번 방을 가기 위해 i 가 필요하다는 의미이므로)
    ...
    for i in check:
        ...
        while queue:
            ...
            for i in queue:              
                ...
                if node in visit:
                    return False
                if node==-1 or node in ok:
                    continue
                    
                temp_list.append(node)
                
                temp=check[node] if node in check else -1
                temp2=res[node]
                queue2.extend([(temp, visit+[node]), (temp2,visit+[node])])
            queue=queue2
        ok=ok|set(temp_list)
